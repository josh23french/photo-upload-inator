...
void CookieJar::load()
{
    ...
    QString dl = QDesktopServices::storageLocation(QDesktopServices::DataLocation);
    dl.chop(2);
    QSettings cookieSettings(dl + QLatin1String("/cookies.ini"), QSettings::IniFormat);
...
{
    qDebug() << "cookies for url: " << url.toString();
...
{
    //qDebug() << "setting cookies! " << url.toString();
    if (!m_loaded)
        load();
